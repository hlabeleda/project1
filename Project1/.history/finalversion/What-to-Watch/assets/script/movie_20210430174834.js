...
movieButton.addEventListener("click", function () { 
    modal.setAttribute("class", "modal is-active");//when play trailer button is clicked, modal is-active is set
});
modalBkg.addEventListener("click", function () {
    modal.setAttribute("class", "modal");//when click outside of video, it resets class back to modal and not modal is active
});
...
$("#myModal").on("hide",function(){
    var leg=$('.iframeVideoPlayer').attr("src");
    $('.iframeVideoPlayer').attr("src",leg);
});
            ...
            fetch(`${apiURL}${ranUrlPage}${randomPage}`).then(function (data) {
                ...
                fetch(movieURL)
                    ...
                    .then(function (response) {
                        ...
                        // $('#show_preview').append(`<iframe width="560" height="315" src="https://www.youtube.com/embed/${response.videos.results[0].key}" id="video" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`)

                        movieButton.addEventListener("click", function () { 
                            $('#show_preview').append(`<iframe class="iframeVideoPlayer" width="560" height="315" src="https://www.youtube.com/embed/${response.videos.results[0].key}" id="video" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`)
